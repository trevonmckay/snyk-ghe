...
namespace SnykGhe.WebhookService.Processing
{
    ...
    public sealed class PullRequestCheckService
    {
        ...
        private readonly FixPullRequestService _fixService;
        private readonly GitHubOptions _gitHub;
        private readonly ILogger<PullRequestCheckService> _logger;
        ...
        public async Task ProcessAsync(ScanRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var scan = await _scanner.ScanAsync(workDir, policy, cancellationToken);
                var (conclusion, title, summary) = BuildReport(scan, policy);
                ...
                try
                {
                    await _fixService.TryOpenAsync(request, policy, scan, credentials.Client, workDir, cancellationToken);
                }
                catch (Exception ex)
                {
